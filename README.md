...
| **设置面板** | 顶栏「设置」：语言（源/目标）、翻译 API（含测试连接）、字幕外观、VAD 灵敏度、说话人分离；**能热生效的热生效、需重启的给提示**，持久化到 `~/.rectheword/settings.yaml` | ✅ 单元 |
| **暖白纸感亮色主题** | 全套 OKLCH 调校亮色（主窗 / 浮窗 / splash 三窗统一），麦(青)/扬(紫) 双色保留，文本对比过 WCAG AA/AAA | ✅ offscreen 截图 |
...
- **用户级设置**：`~/.rectheword/settings.yaml`（设置面板写入，优先于 `config.yaml`）。
  只持久化面板涉及的 `ui` / `api` / `vad` 三个 section，不写模型路径等敏感项。
...
- **主窗口**：无边框（自绘标题栏 + 自绘 —/✕ 控件），可拖动（合成器原生 `startSystemMove`），
  ...
  - **浮窗**：切换浮窗显隐。
- **悬浮窗**：无边框 + 透明 + 置顶，可拖动，**启动时默认隐藏**（由主窗「浮窗」按钮唤起）。
  - **✕ 切回主窗口**：浮窗最小化（进任务栏）+ 唤起主窗（不退出进程）。
  - 退出条件**只有主窗 ✕ 一个**，浮窗不再有独立退出出口。

## 设置面板

顶栏「设置」打开设置对话框，覆盖常用配置，**无需重启即可生效**的部分即时应用：

| 分组 | 可改项 | 生效方式 |
|---|---|---|
| **语言** | 源语言（自动 / 中 / 英 / 日 / 韩 / 德）、目标语言 | 热生效（下一句即用新目标语言） |
| **翻译 API** | Base URL / API Key / Model / 翻译超时 / 纪要超时 + **测试连接** | 热生效（更新客户端 + 重跑 health） |
| **字幕外观** | 主窗外观（light / dark）、浮窗主题、字号、显示原文 | 热生效（浮窗即时刷新） |
| **VAD 灵敏度** | 阈值（0.0–1.0）、最短静音 | 热生效（各声道 VAD 实时 setter） |
| **说话人分离** | 会后离线聚类标注开关 | 热生效 |
| **高级** | ASR 引擎 / 音频源 / 积压自适应 / 会话目录 | **需重启**（面板只读展示，改 `config.yaml`） |

- **持久化**：保存写入 `~/.rectheword/settings.yaml`（用户级，不污染仓库 `config.yaml`）；
  下次启动 `load_config(user_overrides=load_user_settings())` 自动载入，**用户值优先于 config.yaml**。
- **热生效 vs 需重启**：`classify_changes()` 自动对比新旧配置分组；保存后 toast 明确告知
  「已生效：…」与「需重启生效：…」。
- **源语言「自动」** = 信任 ASR 每段返回的 `language`；显式选择则覆盖。

## 配色（暖白纸感亮色）

v3 起默认**亮色**（暖白纸感），由 `color-expert` 方法论推导：OKLCH 感知均匀明度阶梯 +
每对文本/背景经 WCAG 校验 + 麦(青 H205°)/扬(紫 H295°) 色相差 90° 保留。

- **明度阶梯**：bg `#fcf6ee` → panel `#f3ebe3` → panel2 `#e8ded3`（暖白底，纸面阅读感）。
- **文本**：主文 `#251c15`（对底 15.6:1，AAA）；次要 `#64584f`（6.4:1，AA）。
- **强调**：青 `#008390`（白字 4.52:1，AA）/ 紫 `#6e59a6`（白字 5.77:1，AA）。
- 三窗（主窗 / 浮窗 / splash）共用同一组 `TOKENS`；浮窗保留 glass / solid / outline / light 四主题。
- 主窗外观可在设置面板切 `light` / `dark`（`ui.subtitle_theme`）。
